...
PPO算法介绍：
    四个模型：
        策略（演员）模型：待优化的模型，参与参数更新
        价值（评论家）模型：计算当前动作和状态的期望回报，可由奖励模型或策略模型初始化而来，参与参数更新
    ...
    策略：
        在基于人类反馈（RLHF）的强化学习中，策略就是我们要优化的大模型
        从策略进行采样的过程即大模型生成句子的过程
        
    奖励：
        单步奖励：根据当前状态、动作和下一个状态由奖励模型得到的即时奖励，评估当前动作的好坏
        累计奖励：一条完整轨迹的单步奖励累积之和
        折扣奖励：平衡即时奖励和长期奖励之间的关系，使得智能体在决策时不仅要考虑当前的奖励，还要考虑未来的潜在奖励
        
    轨迹：
        轨迹由一系列的状态、动作组成，代表一次完整的采样，即大模型生成一条完整的句子
        
    优化目标：
        通过最大化策略损失来优化策略模型，使得模型生成的句子，有策略才能生成由token组成的句子
        在奖励模型评估下获得更高的奖励，同时通过最大化价值损失来优化价值模型，使得价值模型能够更准确地预测动作的期望回报
        通俗来讲，就是大模型生成的句子获得更多的奖励
"""
...
# 价值（评论家）模型，用于预测每一步（生成token）的动作产生的收益，使用策略（演员）模型进行初始化，并外加一个回归头，输出shape为：(batch_size, seq_len， 1)
...
# 经验池：希望使用历史生成的经验数据，来优化当前模型，因为加入了重要性采样
# 保留新数据，剔除旧数据
...
def generate_samples(prompts, model, max_length, max_new_tokens, n_samples_per_prompt, micro_rollout_batch_size):
    samples_list = []
    model.eval()
    # 从prompts中生成样本，得到模型的推理结果，构建成样本数据结构，包含生成的序列、注意力掩码、动作掩码、动作数等信息
...
if __name__ == "__main__":
    ...
    # 策略模型
    actor_model = AutoModelForCausalLM.from_pretrained('/root/autodl-tmp/Qwen2.5-0.5B-Instruct').to(device)
    # 参考模型
    ref_model = AutoModelForCausalLM.from_pretrained('/root/autodl-tmp/Qwen2.5-0.5B-Instruct').to(device)
    # 奖励模型
    reward_model = AutoModelForSequenceClassification.from_pretrained('/root/autodl-tmp/reward-model-deberta-v3-large-v2').to(device)
    actor_tokenizer = AutoTokenizer.from_pretrained('/root/autodl-tmp/Qwen2.5-0.5B-Instruct')
    reward_tokenizer = AutoTokenizer.from_pretrained('/root/autodl-tmp/reward-model-deberta-v3-large-v2')
    # 价值模型
    # 让价值模型预测出来的值，和实际取得的值之间的差距，越来越小！！！

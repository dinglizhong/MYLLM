...
class Config():
    def __init__(self,
                llm_model_path = '/root/autodl-tmp/Qwen2.5-0.5B-Instruct',
...
if __name__ == '__main__':
    ...
    dataset = MyDataset('/root/autodl-tmp/deepseek_learn/MTP_train/lora_medical.jsonl', tokenizer)
    dataloader = DataLoader(dataset=dataset, batch_size=2, shuffle=True, num_workers=2, collate_fn=MyDataCollator(tokenizer))
    
    optimizer = torch.optim.Adam(model.parameters(), lr=1e-4)
    save_path = './mtp'
    if not os.path.exists(save_path):
        os.makedirs(save_path)
    train(config, model, dataloader, optimizer, writer, device='cuda', epochs=4, print_step=5, save_step=250, save_path='mtp')

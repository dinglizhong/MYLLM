...
if __name__ == "__main__":
    ...
    ref_model = AutoModelForCausalLM.from_pretrained('/root/autodl-tmp/train_llm_from_scratch/saves/sft').eval().to('cuda')
    
    tokenizer = AutoTokenizer.from_pretrained("/root/autodl-tmp/train_llm_from_scratch/tokenizer", use_fast=True)
    ...
    args = TrainingArguments(output_dir='./dpo-1-epoch', 
                            num_train_epochs=2,  # 训练太多轮，模型似乎会输出很多重复内容
                            do_train=True, 
                            ...
                            save_steps=100)          
    dataset = DPODataset('/root/autodl-tmp/train_llm_from_scratch/dataset/dpo.jsonl', tokenizer=tokenizer)

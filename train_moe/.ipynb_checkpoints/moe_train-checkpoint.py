...
# MOE 好处就是极大地增加模型参数量，增加拟合能力
# 推理的时候，只有少数专家被激活，只有一部分参数被激活
# 拟合能力增强的同时，减少推理的计算量
class MoE(nn.Module):
    def __init__(self, config):
        super().__init__()
    ...
    def forward(self, x):
        ...
        # 遍历所有的专家，得到融合了不同专家的输出张量：最终的输出 final_outputs
        for i, expert in enumerate(self.experts):
            ...
            if expert_mask_flat.any():
                ...
                # 将当前专家处理的 token 输入到当前专家模型，得到当前专家的输出
                export_output = expert(expert_input)  # (seq_true, dim)
                # 将输入 token 丢给 sparse 客聘专家的概率分布空间，sparse 客聘专家根据自己的概率分布，评估输入 token 的门控分数
                gate_scores = sparse_logits_flat[expert_mask_flat, i].unsqueeze(1)  # (seq_true) --> (seq_true, 1)
                # 门控分数决定了当前专家的输出对最终输出的贡献程度，门控分数越大，当前专家的输出对最终输出的贡献越大
                # 不同专家有不同的权重，当前专家的输出对最终输出的贡献带有一定的比例（gate_scores）
                weighted_output = export_output * gate_scores  # (seq_true, dim)
                # 将乘以权重之后的专家输出填充到对应位置（对应的当前专家，对应的 token）
...
class DecoderLayer(nn.Module):
    ...
    def forward(
        ...
        # 将奇数层替换为 MOE 层，而偶数层依然是前馈神经网络 MLP 层

...
device = "cuda"
processor = AutoProcessor.from_pretrained('/root/autodl-tmp/llm_related/train_multimodal/siglip-base-patch16-224')
...
#             tokenize=False, \
#             add_generation_prompt=True).replace('<image>', '</tool_call>' + '</tool_call>'*49 + '</tool_call>')

q_text = tokenizer.apply_chat_template([{"role":"system", "content":'You are a helpful assistant.'}, {"role":"user", "content":'第二张图上面有什么\n<image><image>'}], \
            tokenize=False, \
            add_generation_prompt=True).replace('<image>', '<tool_call>'*49 + '\n')

input_ids = tokenizer(q_text, return_tensors='pt')['input_ids']
input_ids = input_ids.to(device)
image1 = Image.open('/root/autodl-tmp/llm_related/train_multimodal/test_images/th1.png').convert("RGB")
image2 = Image.open('/root/autodl-tmp/llm_related/train_multimodal/test_images/th2.png').convert("RGB")
